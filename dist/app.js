...
var UserImpl = class _UserImpl {
  constructor({ uid: uid2, auth: auth2, stsTokenManager, ...opt }) {
...
function listenForCandidates(path, pc) {
  ...
  const callback = (snapshot) => {
    ...
    pc.addIceCandidate(new RTCIceCandidate(snapshot.val())).catch((err) => console.error("\u6DFB\u52A0 ICE candidate \u5931\u8D25:", err));
  };
